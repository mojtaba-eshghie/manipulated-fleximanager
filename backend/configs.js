...
const configEnv = {
  ...
  default: {
    ...
    restServerUrl: "http://0.0.0.0:3443",
    // URL of the UI server
    uiServerUrl: "http://0.0.0.0:3000",
    ...
    mongoBillingUrl: `mongodb://localhost:27017,localhost:27018,localhost:27019/flexibilling?replicaSet=rs`,
    // Billing Redirect OK page url
    //billingRedirectOkUrl: 'http://0.0.0.0/ok.html',
    // Biling config site - this is used as the billing site name in ChargeBee
    //billingConfigSite: 'sdwan-test',
    ...
    //mojtaba: the following is temportary
    ...
    //mojtaba: the following is temporary
    ...
    agentBroker: "192.168.50.2:3000",
    // Whitelist of allowed domains for CORS checks
    corsWhiteList: ["http://0.0.0.0:3000", "http://127.0.0.1:3000"],
    ...
    SwVersionUpdateUrl:
      "https://sandbox.flexiwan.com/Templates/notification_email_${version}.json",
    webHookAddUserUrl: "",
  ...
  production: {
    restServerUrl: "http://0.0.0.0:80",
    uiServerUrl: "http://0.0.0.0:80",
    shouldRedirectHttps: false,
    redirectHttpsPort: 443,
    agentBroker: "0.0.0.0:80",
    ...
    logLevel: "info",
    logUserName: true,
    corsWhiteList: ["http://0.0.0.0:3000", "http:/0.0.0.0:80"],
  },
  hosted: {
    // modify next params for hosted server
    restServerUrl: "http:/0.0.0.0",
    uiServerUrl: "http://0.0.0.0",
    agentBroker: "0.0.0.0:80",
    corsWhiteList: "http:/0.0.0.0, http://0.0.0.0:80",
    billingRedirectOkUrl: "http://0.0.0.0/ok.html",
    shouldRedirectHttps: false,
    redirectHttpsPort: 443,
    validateOpenAPIResponse: false,
    ...
    logFilePath: "/var/log/flexiwan/flexiwan.log",
    reqLogFilePath: "/var/log/flexiwan/flexiwanReq.log",
    SwRepositoryUrl: "http://0.0.0.0/info/flexiwan-router/latest",
    logLevel: "info",
    logUserName: true,
  },
  ...
  appqa01: {
    ...
    SwRepositoryUrl: "http://0.0.0.0/info/flexiwan-router/latest-testing",
    logLevel: "info",
    logUserName: true,
    corsWhiteList: [
      "https://appqa01.flexiwan.com:443",
      "http://appqa01.flexiwan.com:80",
    ],
  },
  ...
  appqa02: {
    ...
    SwRepositoryUrl:
      "https://deb.flexiwan.com/info/flexiwan-router/latest-testing",
    logLevel: "info",
    logUserName: true,
    corsWhiteList: [
      "https://appqa02.flexiwan.com:443",
      "http://appqa02.flexiwan.com:80",
    ],
  },
};
...
class Configs {
  constructor(env) {
    ...
    combinedConfig.userTokenSecretKey =
      process.env.USER_SECRET_KEY || combinedConfig.userTokenSecretKey;
    combinedConfig.deviceTokenSecretKey =
      process.env.DEVICE_SECRET_KEY || combinedConfig.deviceTokenSecretKey;
    combinedConfig.captchaKey =
      process.env.CAPTCHA_KEY || combinedConfig.captchaKey;
    combinedConfig.mongoUrl = process.env.MONGO_URL || combinedConfig.mongoUrl;
    combinedConfig.mongoBillingUrl =
      process.env.MONGO_BILLING_URL || combinedConfig.mongoBillingUrl;
    combinedConfig.mongoAnalyticsUrl =
      process.env.MONGO_ANALYTICS_URL || combinedConfig.mongoAnalyticsUrl;
    combinedConfig.billingApiKey =
      process.env.FLEXIBILLING_API_KEY || combinedConfig.billingApiKey;
    combinedConfig.redisUrl = process.env.REDIS_URL || combinedConfig.redisUrl;
    combinedConfig.webHookAddUserUrl =
      process.env.WEBHOOK_ADD_USER_URL || combinedConfig.webHookAddUserUrl;
    combinedConfig.webHookAddUserSecret =
      process.env.WEBHOOK_ADD_USER_KEY || combinedConfig.webHookAddUserSecret;
    combinedConfig.webHookRegisterDeviceUrl =
      process.env.WEBHOOK_REGISTER_DEVICE_URL ||
      combinedConfig.webHookRegisterDeviceUrl;
    combinedConfig.webHookRegisterDeviceSecret =
      process.env.WEBHOOK_REGISTER_DEVICE_KEY ||
      combinedConfig.webHookRegisterDeviceSecret;
